...
public class HUD {
    ...
    public void resize(int width, int height, RRGame game) {
        // this is basically just to ensure the right column doesn't fly off the end if we shrink the size
        rColumn = Gdx.graphics.getWidth()/2 + (xMargin);
        // this sets the origin of the hudBatch back to the new bottom left of the screen
        game.hudBatch.getProjectionMatrix().setToOrtho2D(0, 0, width, height);
    }
    ...
    public void draw(Batch batch) {
        ...
        if (open) {
            batch.draw(background, 0, Gdx.graphics.getHeight() - ((font.getLineHeight()) * linesbuffered) - yMargin, Gdx.graphics.getWidth(), background.getHeight());
            console = String.join("\n", consoleLines);
            if (console.equals("")) {
